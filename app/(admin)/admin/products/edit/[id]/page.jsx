...
export default function EditProductPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!prodRes.ok) throw new Error(product.error || "Failed to load product");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Failed to preserve changes");
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <p className="text-charcoal-400 font-bold uppercase tracking-widest text-xs">Preloading Masterpiece...</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-6xl mx-auto space-y-10 pb-20 animate-fade-in no-scrollbar">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-charcoal-100 dark:border-charcoal-800 pb-8">
        <div>
          <h2 className="font-heading text-4xl font-extrabold text-ink dark:text-cream-50 leading-tight">Refine Artwork</h2>
          <p className="text-charcoal-500 dark:text-charcoal-400 mt-2 text-lg">Modify the attributes of "{formData.title}"</p>
        </div>
        <div className="flex items-center gap-4">
          ...
          >
            {submitting ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <HiOutlineCheckCircle className="w-5 h-5" />}
            Preserve Changes
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        {/* Left Column: Form Details (Reusing logic from Add Product) */}
        <div className="lg:col-span-2 space-y-10">
          ...
          <section className="bg-white dark:bg-charcoal-800 p-8 rounded-3xl border border-charcoal-100 dark:border-charcoal-700 shadow-sm">
            <div className="flex items-center gap-3 mb-8">
              ...
              <h3 className="font-heading text-xl font-bold text-ink dark:text-cream-50">Core Attributes</h3>
            </div>
          ...
          <section className="bg-white dark:bg-charcoal-800 p-8 rounded-3xl border border-charcoal-100 dark:border-charcoal-700 shadow-sm">
             <div className="flex items-center gap-3 mb-8">
              ...
              <h3 className="font-heading text-xl font-bold text-ink dark:text-cream-50">Narrative & Story</h3>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                <label className="block text-xs font-bold text-charcoal-500 dark:text-charcoal-400 uppercase tracking-widest mb-2">The Full Story</label>
                <textarea 
          ...
          <section className="bg-white dark:bg-charcoal-800 p-8 rounded-3xl border border-charcoal-100 dark:border-charcoal-700 shadow-sm">
             <div className="flex items-center justify-between mb-8">
               <div className="flex items-center gap-3">
                  ...
                  <h3 className="font-heading text-xl font-bold text-ink dark:text-cream-50">Dimensions & Variations</h3>
               </div>
               <button 
                ...
                {variants.map((v, idx) => (
                  <div key={idx} className="p-5 rounded-2xl bg-charcoal-50 dark:bg-charcoal-900/50 flex flex-wrap lg:flex-nowrap items-end gap-4 animate-fade-in group">
                    <div className="flex-1 min-w-[120px]">
                      <label className="block text-[10px] font-bold text-charcoal-400 uppercase mb-1">Size</label>
                      <input 
                    ...
                    <div className="flex-1 min-w-[120px]">
                      <label className="block text-[10px] font-bold text-charcoal-400 uppercase mb-1">Frame</label>
                      <input 
        ...
        <div className="space-y-10">
          ...
          <section className="bg-white dark:bg-charcoal-800 p-8 rounded-3xl border border-charcoal-100 dark:border-charcoal-700 shadow-sm overflow-hidden">
            <h3 className="font-heading text-xl font-bold text-ink dark:text-cream-50 mb-6">Gallery Asset management</h3>
              ...
              <div className="relative group">
                ...
                <div className={`border-2 border-dashed rounded-3xl p-8 transition-all flex flex-col items-center justify-center gap-4 group-hover:bg-charcoal-50 dark:group-hover:bg-charcoal-900/50 ${uploading ? 'bg-charcoal-50 dark:bg-charcoal-900 border-gold-500' : 'border-charcoal-100 dark:border-charcoal-700'}`}>
                  ...
                  <div className="text-center">
                    <p className="text-sm font-bold text-charcoal-700 dark:text-cream-50">Upload New Assets</p>
                    <p className="text-xs text-charcoal-400 mt-1">Enhance visual representation</p>
                  </div>
                </div>
              </div>
          ...
          <section className="bg-white dark:bg-charcoal-800 p-8 rounded-3xl border border-charcoal-100 dark:border-charcoal-700 shadow-sm">
             <div className="flex items-center gap-3 mb-8">
              ...
              <h3 className="font-heading text-xl font-bold text-ink dark:text-cream-50">Value Alignment</h3>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-charcoal-500 dark:text-charcoal-400 uppercase tracking-widest mb-2">Inventory Stock</label>
                  <input 
                ...
                <div>
                   <label className="block text-[10px] font-bold text-charcoal-500 dark:text-charcoal-400 uppercase tracking-widest mb-2">Product SKU</label>
                  <input 
              ...
              <div className="space-y-4 pt-4 border-t border-charcoal-50 dark:border-charcoal-700">
                <label className="flex items-center gap-3 cursor-pointer group">
                  ...
                  <span className="text-sm font-bold text-charcoal-600 dark:text-charcoal-400 group-hover:text-gold-600 transition-colors">Featured Gallery</span>
                </label>
                ...
                <label className="flex items-center gap-3 cursor-pointer group">
                  ...
                  <span className="text-sm font-bold text-charcoal-600 dark:text-charcoal-400 group-hover:text-gold-600 transition-colors">Bestseller Curation</span>
                </label>
              </div>
            </div>
          </section>
